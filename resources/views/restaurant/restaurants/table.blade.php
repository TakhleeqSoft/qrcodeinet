            ...
            <thead>
                <tr role="row">
                    ...
                    @hasanyrole('Super-Admin')
                        <th scope="col">
                            <div class="d-flex justify-content-between">
                                @sortablelink('languages', __('custom.languages'), [], ['class' => 'w-100 text-gray'])
                            </div>
                        </th>
                    @endhasanyrole
                    <th scope="col">
                ...
                @forelse ($restaurants ?? [] as $restaurant)
                    <tr>
                        <td>
                            <h5 class="font-size-14 text-truncate"><a href="ecommerce-product-detail.html"
                                    class="text-dark">
                                    <a href="{{ route('frontend.restaurant', ['restaurant' => $restaurant->slug]) }}"
                                        target="_blank">
                                        <span>{{ $restaurant->name }}</span> <i class=" fas fa-external-link-alt ms-2 "
                                            aria-hidden="true"></i>
                                    </a>
                                </a>
                            </h5>
                    ...
                    <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                        @can('edit restaurants')
                            <a role="button"
                                href="{{ route('restaurant.restaurants.edit', ['restaurant' => $restaurant->id]) }}"
