...
class FrontController extends Controller
{
    ...
    public function restaurantLanguage($restaurant_slug)
    {
        ...
        if ($restaurant->settings->display_language_page == 1) {
            return view('frontend.language-page', compact('restaurant'));
        }else{
            return redirect()->route('frontend.restaurant', $restaurant_slug);
        }
    }

    public function feedbackStore($rt,Request $request)
    {
        $restaurant = Restaurant::where('slug', $rt)->first();
        if (empty($restaurant)) {
            return \response()->json(['status' => 'error', 'message' => 'Restaurant not found']);
        }
        $feedback = new UserFeedback();
        $feedback->restaurant_id = $restaurant->id;
        $feedback->staff = $request->staff;
        $feedback->service = $request->service;
        $feedback->hygiene = $request->hygiene;
        $feedback->overall_experience = $request->overall;
        $feedback->additional_comment = $request->comment;
        $feedback->phone_number = $request->phone;
        $feedback->save();
        return \response()->json(['status' => 'success', 'message' => 'Feedback submitted successfully']);

    }
    public function restaurantLanguageChange($restaurant_slug, $lang)
    {
        $restaurant = Restaurant::where('slug', $restaurant_slug)->with('vendor_setting')->with('settings')->first();
        if (empty($restaurant)) {
            return redirect('/');
        ...
        Cookie::queue('front_dir', $language->direction ?? 'rtl', 24 * 60 * 60 * 365);
        return redirect()->route('frontend.restaurant', $restaurant_slug);
    }
